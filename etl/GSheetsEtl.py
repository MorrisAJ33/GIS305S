...
class GSheetsEtl(SpatialEtl):
    ...
    def extract(self):
        ...
        r = requests.get(super().config_dict.get('remote_url'))
        r.encoding = "utf-8"
        data = r.text
        with open(rf"{super().config_dict.get('proj_dir')}\addresses.csv", "w") as output_file:
            output_file.write(data)

    def transform(self):
        print("Add City, State")

        transformed_file = open(rf"{super().config_dict.get('proj_dir')}\new_addresses.csv", "w")
        transformed_file.write("X,Y,Type\n")
        with open(rf"{super().config_dict.get('proj_dir')}\new_addresses.csv", "w") as partial_file:
            csv_dict = csv.DictReader(partial_file, delimiter=',')
            for row in csv_dict:
                address = row["Street Address"] + " Boulder CO"
                print(address)
                geo_concat = rf"{super().config_dict.get('geocoder_prefix_url')}{address}{super().config_dict.get('geocoder_suffix_url')} "
                print(geo_concat)
    ...
    def load(self):
        ...
        arcpy.env.workspace = rf"{super().config_dict.get('proj_dir')}WestNileOutbreak.gdb\\"
        arcpy.env.overwriteOutput = True

        # Set the local variables
        in_table = rf"{super().config_dict.get('proj_dir')}\new_addresses.csv"
